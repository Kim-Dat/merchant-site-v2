...
const HomePage = () => {
  ...
  const columns: TableProps<DataType>["columns"] = [
    {
      title: (
        <div className="flex items-center gap-3">
          <span>{trans("home.deal-creation-time")}</span>
          <div>
            <Image
    ...
    },
    {
      title: (
        <div className="flex items-center gap-3">
          <span>{trans("home.the-transaction-is-completed")}</span>
          <div>
            <Image
              src={"/icons/arrow-down.svg"}
              alt="arrow down"
  ...
  const handleDataAccount = data.map((item) => ({
    ...
    time: (
      <div>
        <div className="text-sm font-medium text-[#525C67]">
          {item.time.detail}
        </div>
        <div className="text-sm font-medium text-[#525C67]">
          {item.time.date}
    ...
    info: (
      <div>
        <span className="text-sm font-medium text-[#FF1717]">
          {item.info.title}
        </span>
        <p className="text-sm font-medium text-[#525C67]">{item.info.desc}</p>
      </div>
    ),
    ...
    code: (
      <span className="text-sm font-medium text-[#525C67]">{item.code}</span>
    ),
    action: (
    ...
    <div>
      ...
      <div className="shadows-custom mt-3 overflow-hidden rounded-lg bg-white">
        <div className="flex items-center justify-between px-5 py-3">
          ...
          <button className="flex items-center rounded-lg border bg-white px-4 py-2 text-[#344054] shadow-md duration-200 hover:bg-gray-100">
            {trans("home.view-all")}
            <div className="ms-1">
              <Image
